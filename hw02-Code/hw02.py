...
def missions(f):
    ...
    def mission1(f):
        if f(0) == 0 and f(1) == 2:
            ...
            return lambda g: mission2(g(f))#返回一个g(f)函数
        else:
            print('MISSION 1 FAILED')

    def mission2(f):#初次接受一个g(f)函数作为f
        if f(0) == 0 and f(1) == 2:
...
def protected_secret(password, secret, num_attempts):
    ...
    def get_secret(password_attempt):
        "*** YOUR CODE HERE ***"
        chances = num_attempts
        if password_attempt == password and chances > 0:
                print (secret)
              
            
        elif num_attempts > 0 and password_attempt != password:
                chances -= 1
        ...
        return protected_secret(password,secret,chances)
    return get_secret
...
def pow_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda f:lambda x:(n(m)(f))(x)#The problem
    (将m在f上应用n次)